...
if ($request == '/') {
    ...
    $link_changes = preg_match_all('/coupon\',\'http\:\/\/webgiamgia.net\/COut\/(.*?)\',/', $content, $result_1);
    $link_changes = $result_1[1];

    $link_presents = preg_match_all('/href\=\"javascript\:void\(0\)\;\".*?onclick=\"return ShowOfferInPopUp(.*?)\"/', $content, $result_2);
    $link_presents = $result_2[0];

    $i = 0;
    foreach ($link_presents as $link_present) {
        $link_change = 'href="http://webgiamgia.net/COut/' . $link_changes[$i] . '""';
    ...
    if (empty($content)) {
        ...
        foreach ($link_changes as $link_change) {
            ...
            $link[] = '<a href="http://webgiamgia.net' . $request . '?O=' . $link_change[1].'"';
        }
